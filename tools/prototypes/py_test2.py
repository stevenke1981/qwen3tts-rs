...
model = Qwen3TTSModel.from_pretrained(model_dir, device=device, dtype=torch.float16)
...
if hasattr(outputs, 'waveform') or (isinstance(outputs, dict) and 'waveform' in outputs):
    import soundfile as sf
    waveform = outputs['waveform'] if isinstance(outputs, dict) else outputs.waveform
    ...
    print(f"Frames: {outputs.get('frames', 'N/A') if isinstance(outputs, dict) else 'N/A'}")
else:
    print(f"Output type: {type(outputs)}")
    print(f"Keys: {outputs.keys() if hasattr(outputs, 'keys') else 'N/A'}")
